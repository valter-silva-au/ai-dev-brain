...
// Retired names — do not reintroduce a const here without a writer.
//
//   - "mcp_cache.json" (legacy root spelling ".adb_mcp_cache.json") held an MCP
//     health-check TTL cache. Its writer shipped once and was removed with the
//     feature; the const outlived it as an orphan nothing read or wrote, which
//     cost every reader of this file (and of the migration table, the cloud-sync
//     allowlist, and the docs) the effort of deciding it was real. `adb mcp check`
//     deliberately does not cache across runs — you re-run a diagnostic precisely
//     to learn what is true now — so the file is dead data, not a target to
//     relocate. internal.defunctStateFiles deletes any copy left on disk.
//
// The rule this encodes: a const here is a promise that something reads or writes
// the file. Add the writer in the same change as the const, or add neither.
