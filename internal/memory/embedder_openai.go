...
func (e *OpenAIEmbedder) Embed(ctx context.Context, text string) ([]float32, error) {
	...
	// A read failure is a transport fault, not a provider one. Discarding it here
	// pushed a truncated body into json.Unmarshal below, so a connection cut
	// mid-response was reported as "decode response … unexpected end of JSON
	// input" — blaming the provider's JSON and sending the reader to the wrong
	// place entirely.
	respBody, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read response body (status %d): %w", resp.StatusCode, err)
	}
