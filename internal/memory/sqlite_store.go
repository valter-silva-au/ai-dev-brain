...
func (s *SQLiteStore) rebuildIndexFromNodesLocked() {
	...
	// A FIXED seed is the point, not an oversight. HNSW assigns each node a
	// random layer, so an unseeded RNG would give the same corpus a different
	// graph on every rebuild — and this function rebuilds on every Put. Search
	// results would then drift between runs over unchanged data, which makes
	// `adb context memory search` untestable and its ranking unreproducible.
	// Nothing here is a secret, a token, or an identifier a caller sees: the
	// value only decides internal graph shape.
	//nolint:gosec // G404: deterministic layer assignment is required for a reproducible index; not security-relevant
