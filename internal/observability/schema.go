...
//	worktree.removed       task_id, path, reason?
//	knowledge.extracted    task_id, kind, path                    (reserved)
//	agent.session_started  task_id, worktree, bin
//	agent.session_active   task_id, worktree, activity
//	agent.session_ended    task_id, worktree, bin, error?
...
//
// worktree.removed carries an EMPTY task_id for an orphan sweep (`adb task
// worktree prune` / `reconcile --prune`) — the one removal whose worktree
// belonged to no task — plus reason="orphaned", so the empty id reads as an
// answer rather than a hole. Readers of data.task_id must tolerate "": every one
// in this tree already does, either by matching for equality against a non-empty
// id (cli/events.go's filterEvents, so an orphan can never appear in a task's
// timeline) or by skipping "" explicitly (sessiondigest.go).
//
// The five task.* and agent.* consts marked as emissions in
// internal/core/taskmanager.go + internal/cli/launch.go were
...
const (
	...
	// agent.* — payload carries task_id, worktree, bin, and (on end) an
