...
func BuildSessionDigest(log *EventLog, opts SessionDigestOptions) (SessionDigest, error) {
	...
	for _, e := range events {
		...
		// A missing or non-string task_id is an event we cannot attribute to a
		// session, so it is skipped rather than folded under "".
		id, ok := e.Data["task_id"].(string)
		if !ok || id == "" || id == opts.Self {
			continue
		}
	...
	for id, e := range latest {
		// activity is optional: only agent.session_active carries it, so
		// started/ended events legitimately render with an empty verb.
		activity := ""
		if a, ok := e.Data["activity"].(string); ok {
			activity = a
		}
