...
//
// A nil config still falls back to DefaultAlertConfig(), so the zero-config
// behaviour is unchanged; the App seam now passes a config resolved from the
// layered config instead (see ResolveAlertConfig in alertconfig.go).
...
// Config returns the threshold configuration this evaluator evaluates against —
// the resolved one, after the layered config has been applied. It exists so a
// caller can report the thresholds actually in force (and so a test at the App
// seam can assert them) without reaching into the struct.
func (ae *AlertEvaluator) Config() *AlertConfig {
	if ae == nil {
		return nil
	}
	return ae.config
