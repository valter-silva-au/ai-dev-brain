...
// sessionFileMode is 0o600, and is the ONE documented exception to CLAUDE.md's
// 0o644 file mandate.
//
// Session transcripts are the only file class adb writes that routinely contains
// whatever the coding agent READ: an agent session opens .env files, prints
// `aws sts` output, and has credentials pasted into it by a human. models.SessionTurn
// carries verbatim Content plus ToolCalls, and summary.md is LLM prose over the same
// material, so "could this hold a secret?" is answered yes by construction rather
// than by accident.
//
// It applies to every file here, not only the two obvious ones. index.yaml is ids and
// timestamps today, and session.yaml's Metadata is a free-form map[string]string — one
// permission rule per store beats two rules and a note about which file is which.
//
// Directories stay 0o755 deliberately: sessions/ dirs already exist in real
// workspaces (the ticket bootstrap creates them), so tightening those is a separate
// decision with a migration attached. No transcript FILE has ever been written —
// TaskManager.sessionCapturer is assigned and never read — which is exactly why the
// file mode was free to tighten now and would not have been later.
// Chosen in TASK-00039 Batch 4 follow-up; see the G306 note in .golangci.yml.
...
func (fssm *FileSessionStoreManager) saveIndexUnsafe(index *SessionIndex) error {
	...
	// Write to file
	if err := os.WriteFile(fssm.indexPath, data, sessionFileMode); err != nil {
...
func (fssm *FileSessionStoreManager) SaveSession(session *models.CapturedSession) error {
	...
	if err := os.WriteFile(sessionPath, sessionData, sessionFileMode); err != nil {
	...
	if err := os.WriteFile(turnsPath, turnsData, sessionFileMode); err != nil {
	...
	if session.Summary != "" {
		...
		if err := os.WriteFile(summaryPath, []byte(summaryContent), sessionFileMode); err != nil {
