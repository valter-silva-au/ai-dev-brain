...
Dry-run by default; pass --apply to write. --force also overwrites conflicts.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			pi := core.NewFileProjectInitializer(templates.FS)
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if _, err := os.Stat(claudePath); os.IsNotExist(err) {
				claudeContent := "# Claude Context\n\n" +
					...
					"- `adb context build` - Regenerate this file\n" +
					"- `adb metrics` - View workspace metrics\n" +
					"- `adb alerts` - What needs attention\n"
...
- A document-program pack under programs/ (default: ` + core.DefaultProgram + `;
  choose others with --with-program, opt out with --no-programs)`,
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			targetPath := "."
			if len(args) > 0 {
			...
			initializer := core.NewFileProjectInitializer(templates.FS)
			...
			if programs := core.ProvisionedPrograms(options); len(programs) > 0 {
				fmt.Printf("   • Document program(s): %s (programs/)\n", strings.Join(programs, ", "))
			}
			...
			fmt.Println("   adb task list                  # See what is in flight")
	...
	cmd.Flags().StringSliceVar(&withPrograms, "with-program", nil,
		fmt.Sprintf("Document-program pack(s) to provision under programs/ (repeatable; default %q)", core.DefaultProgram))
	cmd.Flags().BoolVar(&noPrograms, "no-programs", false, "Do not provision any document-program pack")
