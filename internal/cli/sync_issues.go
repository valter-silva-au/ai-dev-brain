...
_local tickets, absolute or relative local paths, and Amazon-internal repos
(code.aws.dev, code.amazon.com) are skipped.
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			s := &issuesync.Syncer{
				Body: func(t models.Task) string {
					...
					b, rerr := os.ReadFile(filepath.Join(dir, "context.md"))
					if rerr != nil {
						// Best-effort by contract: a ticket with no context.md
						// syncs an empty body rather than failing the whole run.
						return ""
					}
					return string(b)
				},
