...
func NewMetricsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Compute metrics — windowed when --since is given, else all-time.
			...
			if since != "" {
				...
				// Tally ONLY events within the window. (The old code replayed the
				// whole log and merely blanked the result when the newest event
				// predated the cutoff, so --since returned all-time counts unless
				// the workspace was idle — #153.)
				cutoff := time.Now().UTC().Add(-duration)
				metrics, err = App.MetricsCalculator.ComputeMetricsSince(cutoff)
				if err != nil {
...
// parseDuration is the CLI's spelling of adb's day-aware duration rule, shared by
// `adb metrics --since`, `adb events query --since` and `adb task timeline --since`.
// It delegates to observability.ParseDuration, which is the canonical
// implementation — the alert thresholds resolved in internal/app.go read the same
// rule, and two copies of it is how "7d" comes to mean two different things.
func parseDuration(s string) (time.Duration, error) {
	return observability.ParseDuration(s)
}
