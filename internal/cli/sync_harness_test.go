...
// not clobber it.
//
// The CLAUDE_CONFIG_DIR t.Setenv calls below stay after the NewAppIsolated
// migration: that variable is a WRITE TARGET resolved by resolveClaudeConfigDir
// (sync.go), not one of the App's config tiers, so isolation has no say over it.
func TestSyncClaudeUser_InstallsHarness(t *testing.T) {
	ws := t.TempDir()
	app, err := internal.NewAppIsolated(ws)
	if err != nil {
