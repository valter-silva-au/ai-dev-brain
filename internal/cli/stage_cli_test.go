...
func runADB(t *testing.T, args ...string) error {
	...
	cmd := NewRootCmd()
	cmd.SetArgs(args)
	// Silence cobra's own output during tests. Never wrap a live fd here:
	// os.NewFile(0, …) attaches a finalizer that closes fd 0 at GC time, and
	// the kernel then reuses that fd for whatever file another test has open —
	// the source of the suite-wide "bad file descriptor" flake.
...
func TestStageCLI_OrgInitiativeStage_EndToEnd(t *testing.T) {
	...
	reopened, err := internal.NewAppIsolated(tmp)
	if err != nil {
		t.Fatalf("reopen NewAppIsolated: %v", err)
	}
