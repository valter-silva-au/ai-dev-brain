...
func newSchedulerRestartCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// A failed stop leaves the old daemon running, so starting on top of
			// it is never what was asked for — abort instead of reporting the
			// start's confusing "already running" error.
...
func schedulerDaemonStart() error {
	...
	// Release detaches the daemon from this process. The daemon is already running
	// and its PID recorded by this point, so a failure here does not fail the
	// start — but it is reported, because the consequence (adb waiting on a child
	// it meant to disown) is not something to discover without a clue.
	if err := cmd.Process.Release(); err != nil {
		fmt.Fprintf(os.Stderr, "Warning: could not detach scheduler process %d: %v\n", pid, err)
	}
...
// signalStop is the process-termination seam, held as a var so a test can drive
// schedulerDaemonStop's error handling without a real daemon to signal (and
// without SIGTERMing the test binary). Same swap idiom as agentLauncher.
var signalStop = stopProcess

func swapSignalStop(fake func(*os.Process) error) func() {
	previous := signalStop
	signalStop = fake
	return func() { signalStop = previous }
}
...
func schedulerDaemonStop() error {
	...
	if err != nil {
		// A PID we cannot even reference has nothing left to stop. Say so:
		// this used to return nil having printed nothing at all.
		removeSchedulerPIDFile()
		fmt.Printf("Scheduler is not running (stale PID %d: %v)\n", pid, err)
		//nolint:nilerr // an unreferenceable PID is "already stopped", not a failure to stop
		return nil
	}
	if err := signalStop(p); err != nil {
		removeSchedulerPIDFile()
		// The one reachable failure is the daemon exiting between the liveness
		// probe above and this signal, which os.Process.Signal reports as
		// ErrProcessDone — that really is a stop. Anything else (a PID we are not
		// permitted to signal, say) means the daemon is still running, and must
		// not read as "✓ stopped" with an exit status of 0.
		if errors.Is(err, os.ErrProcessDone) {
			fmt.Printf("✓ Scheduler stopped (PID %d was not running)\n", pid)
			return nil
		}
		return fmt.Errorf("stop scheduler (PID %d): %w", pid, err)
	}
	removeSchedulerPIDFile()
...
// removeSchedulerPIDFile drops the PID file, tolerating an already-absent one.
// A PID file that could NOT be removed is worth naming rather than discarding:
// `adb scheduler start` refuses to run for as long as one looks live.
func removeSchedulerPIDFile() {
	if err := os.Remove(schedulerPIDPath()); err != nil && !os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Warning: could not remove %s: %v\n", schedulerPIDPath(), err)
	}
}
...
// schedulerPullRepos runs one unattended repos-pull. FetchOnly: the
// interactive `adb repo pull` checks out the default branch, but a background
// job that fires every 15 minutes must never switch a repo off a deliberately
// checked-out branch — it only freshens origin refs. The pull engine is
// injectable so tests can assert the opts without touching git.
func schedulerPullRepos(basePath string, pull func(string, integration.PullOpts) (integration.PullSummary, error)) (string, error) {
	summary, err := pull(basePath, integration.PullOpts{FetchOnly: true})
	if err != nil {
		return "", err
	}
...
func schedulerRunForeground() error {
	...
	deps := scheduler.Deps{
		...
		PullRepos: func(ctx context.Context) (string, error) {
			return schedulerPullRepos(App.BasePath, integration.PullAllRepos)
		},
