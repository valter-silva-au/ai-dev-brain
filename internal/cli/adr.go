...
// parseADRNumber accepts either "12" or "adr:0012"/"ADR-12" forms. The graph
// prefix comes from models.ADRGraphPrefix — the same const models.ADR.GraphID
// formats with — so this parser cannot drift from the ids the tool prints.
func parseADRNumber(s string) (int, error) {
	s = strings.TrimSpace(s)
	s = strings.TrimPrefix(strings.ToLower(s), models.ADRGraphPrefix)
