...
func newComplianceListCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			frameworks, err := core.ComplianceFrameworks(templates.FS)
			if err != nil {
				return err
			}
...
func newComplianceScaffoldCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, err := core.ScaffoldComplianceFramework(templates.FS, framework, destDir, core.HarnessInstallOptions{DryRun: dryRun, Force: force})
			if err != nil {
