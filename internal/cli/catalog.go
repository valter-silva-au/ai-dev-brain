...
func NewCatalogCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "catalog",
		Short: "Generated entity catalog (orgs, initiatives, tickets, nodes, metrics, adrs, debt)",
...
func newCatalogShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch kind {
			case "", "orgs", "initiatives", "tickets", "nodes", "metrics", "adrs", "debt":
			default:
				return fmt.Errorf("unknown --kind %q (want orgs|initiatives|tickets|nodes|metrics|adrs|debt)", kind)
			}
	...
	cmd.Flags().StringVar(&kind, "kind", "", "limit to one kind: orgs|initiatives|tickets|nodes|metrics|adrs|debt")
	return cmd
}
...
func renderCatalog(cat *models.Catalog, kind string) {
	...
	if show("debt") {
		fmt.Printf("Tech debt (%d):\n", len(cat.Debt))
		for _, d := range cat.Debt {
			area := d.Area
			if area == "" {
				area = "-"
			}
			fmt.Printf("  %-16s %-3s %-8s area=%-16s %s (edges=%d)\n",
				d.ID, d.Priority, d.Status, area, d.Title, d.Edges)
		}
	}
	if kind == "" {
		s := cat.Summary
		fmt.Printf("\nTotal: %d orgs, %d initiatives, %d tickets, %d nodes, %d metrics, %d adrs, %d debt, %d edges\n",
			s.Orgs, s.Initiatives, s.Tickets, s.IngestedNodes, s.Metrics, s.ADRs, s.Debt, s.Edges)
	}
}
