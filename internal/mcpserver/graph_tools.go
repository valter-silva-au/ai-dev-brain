...
func handleSearchKnowledge(app *internal.App) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if !configured {
			return jsonResult(map[string]any{
				"configured": false,
				"notice":     "vector memory is not configured for this workspace (no knowledge base found — enable hooks.memory or index ticket knowledge with `adb context memory index`)",
				"count":      0,
				"hits":       []knowledgeHit{},
			})
		}
