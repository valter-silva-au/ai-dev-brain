...
func TestNewRegistersV3FoundationTools(t *testing.T) {
	t.Parallel()

	mcpServer := New(nil, "test")
	for _, name := range []string{
		"adb_workspace_initialize",
		"adb_workspace_doctor",
		"adb_organization_initialize",
		"adb_organization_list",
		"adb_organization_show",
		"adb_organization_update",
		"adb_organization_adopt",
		"adb_organization_move",
		"adb_organization_archive",
		"adb_organization_validate",
		"adb_repository_add",
		"adb_repository_adopt",
		"adb_repository_list",
		"adb_repository_show",
		"adb_repository_health",
		"adb_repository_fetch",
		"adb_repository_update",
		"adb_repository_move",
		"adb_repository_archive",
		"adb_repository_worktree_list",
		"adb_repository_worktree_repair",
		"adb_repository_worktree_prune",
	} {
		if mcpServer.GetTool(name) == nil {
			t.Fatalf("v3 foundation tool %q is not registered", name)
		}
	}
}
...
func TestHandleStart_ReportsActualStatus(t *testing.T) {
	t.Parallel()
	// Isolated for the same reason as every other App in this package: a
	// t.TempDir() workspace pins only the REPO tier, while an un-isolated App's
	// GLOBAL tier is the developer's ~/.taskconfig, which feeds this App's hook
	// gates, task_id_prefix, and memory config. Nothing here asserts on those
	// TODAY, so this is consistency and future-proofing rather than a live bug.
	// See the isolation note at the top of graph_tools_test.go.
