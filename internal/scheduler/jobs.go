...
type Deps struct {
	...
	// PullRepos runs an `adb repo pull` once. Should return a short
		...
		},
		{
			...
			Run: func(ctx context.Context) error {
				for _, path := range deps.LogFiles {
					...
					rotated, err := rotateIfLarge(path, rotateThreshold)
					if err != nil {
...
// size is >= threshold. Returns true if rotation happened. Siblings beyond
// keepRotations are deleted.
//
// threshold is a parameter because tests cannot practically write
// rotateThreshold (50 MiB) worth of bytes; the retention depth is not, because
// every caller wants keepRotations and a second knob nothing varies is a
// configuration surface that only looks like one.
func rotateIfLarge(path string, threshold int64) (bool, error) {
	const keep = keepRotations
