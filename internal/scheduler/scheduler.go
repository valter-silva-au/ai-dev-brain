...
func Run(ctx context.Context, opts RunOptions) error {
	...
	// load() already returns nil for a MISSING file, so the errors that reach
	// here are a read failure or a corrupt YAML body — not "first start". Both
	// leave every job looking never-run (so RunOnStart fires all of them at
	// once), which is confusing without this line. Non-fatal: a broken state
	// file must not stop the daemon.
	if err := states.load(); err != nil {
		logf("could not load scheduler state from %s: %v", opts.StateFile, err)
	}
...
func runJobLoop(
	...
	// recordState persists a state mutation and logs a failure instead of
	// discarding it — a state write that never lands makes `adb scheduler list`
	// lie and makes RunOnStart re-run the job after a restart.
	recordState := func(mutator func(*State)) {
		if err := states.update(job.Name, mutator); err != nil {
			logf("job %s: %v", job.Name, err)
		}
	}

	invoke := func() {
		if !mu.TryLock() {
			recordState(func(s *State) { s.Skipped++ })
