...
type MockBacklogStore struct {
	...
	loadErr error
	saveErr error
	// removeErr fails RemoveTask ONLY, which saveErr cannot express: saveErr
	// also fails AddTask, so a rollback-path test could never get a task into
	// the store to then fail removing it.
	removeErr error
}
...
// TestTaskManager_Create_FailedRollbackIsReported pins that a rollback step
// which itself fails leaves a trace. Create's rollbacks used to discard their
// errors with a bare `_ =`, so when the worktree failed AND the backlog row
// could not be removed, the user got "failed to create worktree" and a
// permanent orphan row that `adb task list` would keep showing for a ticket
// with no directory and no worktree — with nothing anywhere saying why.
//
// The returned error must still be the one that TRIGGERED the rollback: the
// rollback failure is additional information, never a replacement.
func TestTaskManager_Create_FailedRollbackIsReported(t *testing.T) {
	tm, backlogStore, _, worktreeCreator, _, _ := createTestTaskManager(t)
	worktreeCreator.createErr = fmt.Errorf("git worktree add exploded")
	backlogStore.removeErr = fmt.Errorf("backlog.yaml is read-only")

	var err error
	stderr := captureConfigStderr(t, func() {
		_, err = tm.Create(CreateTaskOpts{Title: "Rollback Task", TaskType: models.TaskTypeFeat, Repo: "github.com/test/repo"})
	})

	if err == nil {
		t.Fatal("Create should fail when the worktree cannot be created")
	}
	if !strings.Contains(err.Error(), "git worktree add exploded") {
		t.Errorf("returned error should be the one that triggered the rollback, got %v", err)
	}
	if !strings.Contains(stderr, "backlog.yaml is read-only") {
		t.Errorf("failed rollback step not reported on stderr; got %q", stderr)
	}
	if !strings.Contains(stderr, "remove backlog entry") {
		t.Errorf("stderr should name the rollback step that failed; got %q", stderr)
	}
}
