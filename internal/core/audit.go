	...
	return []auditControl{
		{
			...
			check: func(base string) (models.AuditStatus, string) {
				...
				return models.AuditFail, "no .gitleaks.toml — configure a secret scanner (adb archive push's gate uses gitleaks)"
			},
		},
		{
...
// ValidAuditFrameworks is the closed set of compliance frameworks the audit
// knows attestation controls for.
//
// It is exported so the CLI can REJECT an unknown value. Passing one through
// silently was worse than a crash: `adb audit security --framework pci` ran the
// deterministic controls and printed a normal-looking report, so a reader could
// reasonably believe a PCI audit had happened.
func ValidAuditFrameworks() []string {
	return []string{"soc2", "gdpr", "hipaa"}
}

// IsValidAuditFramework reports whether name is a known framework.
func IsValidAuditFramework(name string) bool {
	for _, f := range ValidAuditFrameworks() {
		if f == name {
			return true
		}
	}
