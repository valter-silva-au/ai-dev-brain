...
// initiative registries, the backlog, the ingested-node registry, and the
// metric, ADR, and tech-debt registries. Each method returns the entities in
// their registry order — the builder sorts deterministically.
...
func (b *catalogBuilder) Build() (*models.Catalog, error) {
	...
	debt, err := b.source.Debt()
	if err != nil {
		return nil, fmt.Errorf("catalog: load tech debt: %w", err)
	}
	...
	cat := &models.Catalog{
		...
		Debt:          make([]models.CatalogDebt, 0, len(debt)),
	}
	...
	// A tech-debt item is a debt:DEBT-NNNN node, keyed by its graph id like a
	// metric or an ADR — so the registry `adb debt` writes is readable from the
	// catalog and traversable with `adb graph neighbors`.
	for _, d := range debt {
		cat.Debt = append(cat.Debt, models.CatalogDebt{
			ID:       d.GraphID(),
			Title:    d.Title,
			Priority: string(d.Priority),
			Status:   string(d.Status),
			Area:     d.Area,
			Edges:    degree(d.GraphID()),
		})
	}
	...
	// Debt ids are zero-padded (DEBT-0001), so sorting by graph id IS numeric
	// order. The catalog is an inventory, not a triage queue — `adb debt list`
	// owns triage order (open first, then priority).
	sort.Slice(cat.Debt, func(i, j int) bool { return cat.Debt[i].ID < cat.Debt[j].ID })
