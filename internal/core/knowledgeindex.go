...
// completion) with an on-demand FULL-workspace pass driven by `adb context memory index`.
//
...
func (ki *KnowledgeIndexer) indexTaskFiles(ctx context.Context, t models.Task) (int, error) {
	...
	if dir == "" {
		...
		if err != nil {
			...
			//nolint:nilerr // deliberate: an unresolvable ticket dir contributes 0 documents, it does not fail the pass
			return 0, nil
		}
		dir = resolved
	}
