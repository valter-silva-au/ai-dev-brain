...
func evaluateGate(bundle gateBundle, ev gateEval) models.GateState {
	...
	for _, it := range bundle.Items {
		...
		case gateDeterministic:
			fallthrough
		default:
			// Deterministic, and the fail-safe for an unset or unrecognised Kind:
			// the gateKind zero value is gated on its artifact like a deterministic
			// item, so a bundle that forgot its Kind still BLOCKS on missing
			// evidence rather than waving it through. gateDeterministic is spelled
			// out so the switch is exhaustive over gateKind (a future kind then
			// shows up as a lint failure here instead of silently landing in this
			// branch and being gated on an Artifact it does not have).
