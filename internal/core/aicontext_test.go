...
func TestAIContextGenerator_Generate(t *testing.T) {
	...
	agentsPath := filepath.Join(tmpDir, CanonicalInstructionFile)
	...
	if !strings.Contains(contentStr, "TASK-001") {
		t.Error("Active task TASK-001 not found in AGENTS.md")
	}
	if !strings.Contains(contentStr, "TASK-002") {
		t.Error("Active task TASK-002 not found in AGENTS.md")
	}
	...
	// Verify context_state.yaml was created (under .adb/ as of #186/#189).
	statePath := filepath.Join(tmpDir, ".adb", "context_state.yaml")
	if _, err := os.Stat(statePath); os.IsNotExist(err) {
		t.Fatal(".adb/context_state.yaml was not created")
	}
...
func TestAIContextGenerator_WithDecisions(t *testing.T) {
	...
	if !strings.Contains(contentStr, "Use PostgreSQL") {
		t.Error("Decision title not found in AGENTS.md")
	}
	if !strings.Contains(contentStr, "PostgreSQL for data storage") {
		t.Error("Decision description not found in AGENTS.md")
	}
}
...
`
	...
	if !strings.Contains(contentStr, "tickets/TASK-001/sessions/session-001.md") {
		t.Error("Session file path not found in AGENTS.md")
	}
	if !strings.Contains(contentStr, "This is a test session") {
		t.Error("Session content not found in AGENTS.md")
	}
}

// TestAIContextGenerator_SessionsSurviveUnstatableFile pins that a session path
// the walk yielded but os.Stat cannot resolve does not take the whole generator
// down. The recent-sessions sort used to os.Stat both sides inside the less
// function and call ModTime() on the result, so a nil FileInfo was a
// nil-pointer PANIC inside sort.Slice — not a degraded section, a dead
// `adb context build`. Two ways to reach it in a real workspace: a broken
// symlink under a sessions/ dir (used here, since it is deterministic), and a
// session file deleted between the walk and the sort.
func TestAIContextGenerator_SessionsSurviveUnstatableFile(t *testing.T) {
	tmpDir := t.TempDir()
	setupTestEnvironment(t, tmpDir)
	...
	// A readable session, and a dangling symlink that sorts BEFORE it so the
	// unstatable side is the one the comparison reaches second.
	if err := os.WriteFile(filepath.Join(sessionsDir, "good.md"), []byte("# Session\n\nreadable session body\n"), 0o644); err != nil {
		t.Fatalf("Failed to write session file: %v", err)
	}
	if err := os.Symlink(filepath.Join(sessionsDir, "does-not-exist.md"), filepath.Join(sessionsDir, "broken.md")); err != nil {
	...
	content, err := os.ReadFile(filepath.Join(tmpDir, CanonicalInstructionFile))
	if err != nil {
		t.Fatalf("Failed to read %s: %v", CanonicalInstructionFile, err)
	}
	// The readable session is still reported — degrading, not skipping the section.
	if !strings.Contains(string(content), "readable session body") {
		t.Error("readable session body missing: the unstatable sibling should be skipped, not the whole section")
	}
}
...
`
	...
	if !strings.Contains(contentStr, "001-microservices.md") {
		t.Error("ADR filename not found in AGENTS.md")
	}
}
