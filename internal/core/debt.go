...
// DebtManager owns the architecture-audit / tech-debt registry. Each item is a
// debt:DEBT-NNNN node in the typed graph (#109) carrying whatever typed links it
// declares, mirroring how an ADR participates as adr:NNNN.
type DebtManager interface {
	// Add records a new open debt item (priority defaults to P2). links are the
	// typed graph edges the item declares (nil is the ordinary case — the item is
	// then a degree-0 node).
	Add(title, area, note string, priority models.Priority, links []models.Link) (models.DebtItem, error)
...
func (m *debtManager) Add(title, area, note string, priority models.Priority, links []models.Link) (models.DebtItem, error) {
