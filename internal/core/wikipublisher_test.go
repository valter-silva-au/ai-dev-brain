...
// fakeClassifier returns fixed org/initiative names (test seam for the
// namespacing frontmatter).
type fakeClassifier struct{ org, initiative string }

func (f fakeClassifier) Classify(taskID string) (string, string) { return f.org, f.initiative }
...
// TASK-00049: org/initiative names are free-form user strings; a name with
// ": " must not break the page's YAML frontmatter. The emitted frontmatter
// must strict-parse and round-trip.
func TestWikiPublisher_HostileOrgInitiative(t *testing.T) {
	base := t.TempDir()
	seedKnowledge(t, base, "TASK-00049", &models.ExtractedKnowledge{
		TaskID:    "TASK-00049",
		Decisions: []models.Decision{{ID: "D", Title: "X", Status: "decided"}},
	})
	p := newTestPublisher(base)
	p.SetClassifier(fakeClassifier{org: "valter-silva-au: personal", initiative: "Learn pipeline: channel-catalog"})

	outDir := filepath.Join(base, "out")
	if _, err := p.PublishAll(outDir); err != nil {
		t.Fatal(err)
	}
	// A namespaced page lands under org/initiative dirs; find it.
	var pagePath string
	filepath.Walk(outDir, func(path string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() && strings.HasSuffix(path, "task-00049-knowledge.md") {
			pagePath = path
		}
		return nil
	})
	if pagePath == "" {
		t.Fatal("knowledge page not written")
	}
	...
	if end == -1 {
		t.Fatalf("no frontmatter close:\n%s", content)
	}
	fm := content[4 : 4+end]
	var m map[string]any
	if err := yaml.Unmarshal([]byte(fm), &m); err != nil {
		t.Fatalf("frontmatter not valid YAML:\n%s\n%v", fm, err)
	}
	if m["org"] != "valter-silva-au: personal" || m["initiative"] != "Learn pipeline: channel-catalog" {
		t.Errorf("frontmatter did not round-trip: org=%#v initiative=%#v", m["org"], m["initiative"])
	}
}
