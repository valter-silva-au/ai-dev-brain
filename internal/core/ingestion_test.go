...
// TestIngest_Submit_RejectsNonCanonicalEdgeType is the WRITE half of the closed
// #109 vocabulary at the ingestion boundary. Proposals are authored by an
// extraction skill — an LLM — which is exactly the surface that invents a
// plausible-sounding "mentions" or "owns". The whole batch must fail with nothing
// committed (Submit validates up front by design), so a bad type is an authoring
// error rather than a partially-landed one.
func TestIngest_Submit_RejectsNonCanonicalEdgeType(t *testing.T) {
	_, ps, _, edges, m := newTestIngest(true)
	_, err := m.Submit([]models.EntityProposal{
		{ID: "ok", RawID: "raw-1", Kind: models.ProposalEdge, Confidence: 0.99, From: "TASK-1", Edge: &models.Link{Type: models.EdgeRelatesTo, Target: "TASK-2"}},
		{ID: "bad", RawID: "raw-1", Kind: models.ProposalEdge, Confidence: 0.99, From: "TASK-3", Edge: &models.Link{Type: models.EdgeType("mentions"), Target: "TASK-4"}},
	}, 0.8)
	if err == nil {
		t.Fatal("Submit accepted a non-canonical edge type, want a rejection")
	}
	if !strings.Contains(err.Error(), "mentions") {
		t.Errorf("error = %v, want it to name the rejected type", err)
	}
	if len(edges.edges) != 0 || len(ps.queue) != 0 || len(ps.ledger) != 0 {
		t.Errorf("partial commit: edges=%+v queue=%+v ledger=%+v, want nothing committed", edges.edges, ps.queue, ps.ledger)
	}
}

// TestIngest_Accept_RejectsNonCanonicalEdgeType closes the remaining way in. A
// proposal cannot reach the queue through Submit with a bad type any more, but a
// hand-edited (or older-build) ingested/queue.yaml can still carry one — and
// accepting it is a write to the graph's source of truth. It must be refused, and
// the proposal must survive in the queue rather than be consumed by the failure.
func TestIngest_Accept_RejectsNonCanonicalEdgeType(t *testing.T) {
	_, ps, _, edges, m := newTestIngest(true)
	// Seed the queue directly, bypassing Submit — this is the hand-edited case.
	ps.queue = []models.EntityProposal{
		{ID: "legacy", RawID: "raw-1", Kind: models.ProposalEdge, Confidence: 0.2, Status: models.ProposalPending, From: "TASK-1", Edge: &models.Link{Type: models.EdgeType("mentions"), Target: "TASK-2"}},
	}
	// READ stays tolerant: the review queue still shows it.
	pending, err := m.Pending()
	if err != nil {
		t.Fatalf("Pending(): %v", err)
	}
	if len(pending) != 1 || pending[0].Edge.Type != models.EdgeType("mentions") {
		t.Fatalf("Pending() = %+v, want the non-canonical proposal listed verbatim", pending)
	}
	// WRITE is refused.
	if _, err := m.Accept("legacy"); err == nil {
		t.Fatal("Accept applied a non-canonical edge type, want a rejection")
	}
	if len(edges.edges) != 0 {
		t.Errorf("edges = %+v, want none minted", edges.edges)
	}
	if len(ps.queue) != 1 {
		t.Errorf("queue = %+v, want the proposal still queued (never silently lost)", ps.queue)
	}
}
