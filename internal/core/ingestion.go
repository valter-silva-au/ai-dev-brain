...
func (m *ingestManager) apply(p models.EntityProposal) error {
	...
	case models.ProposalEdge:
		...
		//
		// Submit() already rejects a non-canonical type at proposal time, so this
		// gate exists for the one remaining way in: accepting a proposal that was
		// hand-written into ingested/queue.yaml (or queued by an older build).
		// Landing an edge IS the write, so it is refused here — while `adb ingest
		// review` keeps listing the proposal, which is the read half.
		//
		// The error is NOT prefixed with the proposal id: both callers already name
		// it (Accept's "apply proposal %q", Submit's re-queue path), and prefixing
		// here produced `apply proposal "p1": proposal "p1": unknown edge type …`.
		if err := p.Edge.Type.Validate(); err != nil {
			return err
		}
