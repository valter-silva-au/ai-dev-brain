...
// SetStandingRules injects the standing DOs/DONTs content rendered into every
// worktree Tier-0 task-context.md. Post-construction setter for the same
// reason as SetInitiativeResolver: the content is a cross-cutting harness
// asset owned by the composition root, and an empty value degrades gracefully
// (core falls back to a terse stub) so tests never need it.
func (tm *TaskManager) SetStandingRules(rules string) {
	tm.standingRules = rules
}
...
// warnRollback reports a failed rollback step on stderr instead of discarding
// it. Create's rollbacks run on a path that is ALREADY returning an error, so a
// rollback failure must never replace the error that caused it — but it is not
// nothing either: a rollback that half-completes leaves the workspace
// inconsistent (an orphan backlog row for a ticket dir that was never
// bootstrapped, or a stray registered worktree), and the user can only clean
// that up if they are told it happened. A silent `_ =` here is how a workspace
// acquires a task id that `adb task show` finds and nothing else does.
func warnRollback(step string, err error) {
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: rollback step %q failed, workspace may be inconsistent: %v\n", step, err)
	}
}
...
func (tm *TaskManager) Create(opts CreateTaskOpts) (*models.Task, error) {
	...
	bootstrapConfig := BootstrapConfig{
		...
		StandingRules:      tm.standingRules,
	...
	if err != nil {
		...
		warnRollback("remove backlog entry "+taskID, tm.backlogStore.RemoveTask(taskID))
	...
	if opts.Repo != "" && tm.worktreeCreator != nil && task.Type != models.TaskTypeWork {
		...
		if exists, berr := tm.worktreeCreator.BranchExists(opts.Repo, branchName); berr == nil && exists {
			...
			// A BranchExists error is deliberately read as "not taken" here, the
			// same way the outer `berr == nil && exists` guard does: the probe is
			// an optimisation over letting `git worktree add -b` fail, so when the
			// probe itself is broken we proceed and let git be the authority.
			//nolint:errcheck // an unreadable probe degrades to "assume free"; git is the real check
			if taken, _ := tm.worktreeCreator.BranchExists(opts.Repo, disambiguated); taken {
				warnRollback("remove ticket dir "+result.TaskDir, os.RemoveAll(result.TaskDir))
				warnRollback("remove backlog entry "+taskID, tm.backlogStore.RemoveTask(taskID))
	...
	if err := tm.backlogStore.UpdateTask(*task); err != nil {
		...
		if tm.worktreeRemover != nil && task.WorktreePath != "" {
			warnRollback("remove worktree "+task.WorktreePath, tm.worktreeRemover.RemoveWorktree(task.WorktreePath, true))
		}
		warnRollback("remove ticket dir "+result.TaskDir, os.RemoveAll(result.TaskDir))
		warnRollback("remove backlog entry "+taskID, tm.backlogStore.RemoveTask(taskID))
...
func (tm *TaskManager) Archive(taskID string, opts ArchiveOptions) error {
	...
	// Generate handoff.md from template. The template renders the close-out
	// sections (Summary / What Was Done / Key Decisions / Open Items / Next
	// Steps); the close-out itself lives in notes.md and knowledge/decisions.yaml
	// (written while the work was live), so the generated handoff only points at
	// them plus any decision-of-record entries. Before TASK-00031 this call
	// passed metadata only and every section rendered "<no value>".
	handoffData := map[string]interface{}{
		...
		"Summary":        fmt.Sprintf("Closed %s — see the ticket files: context.md (why), design.md (how), notes.md (progress log).", time.Now().UTC().Format("2006-01-02")),
		"CompletedItems": []string{},
		"Decisions":      []string{},
		"OpenItems":      []string{},
		"NextSteps":      "None recorded — deferred items are tracked in their own tickets.",
		"References":     "notes.md · context.md · knowledge/decisions.yaml",
	}

	handoffContent, err := tm.templateManager.RenderBytes(TemplateTypeHandoff, stampTemplateData(tm.templateManager, TemplateTypeHandoff, handoffData))
	if err != nil {
	...
	if !opts.KeepWorktree && tm.worktreeRemover != nil && task.WorktreePath != "" {
		...
		if err := tm.worktreeRemover.RemoveWorktree(removedPath, opts.Force); err != nil {
			fmt.Fprintf(os.Stderr, "Warning: worktree left in place (%v). Commit/push and `adb task worktree remove %s`, or re-archive with --force.\n", err, taskID)
			keptWorktree = true
		} else {
			if tm.eventLogger != nil {
...
func (tm *TaskManager) Unarchive(taskID string) error {
	...
	// The ticket has left; prune whatever empty nesting it was the last occupant
	// of (e.g. _archived/github.com/acme/thing/). Cosmetic, and deliberately
	// non-fatal: an un-pruned directory must never fail an unarchive that
	// already succeeded.
	tm.pruneEmptyArchivedDirs(filepath.Dir(archivedTaskDir))
...
// pruneEmptyArchivedDirs removes the empty nesting an unarchived ticket left
// behind under tickets/_archived/, walking upward from dir and stopping at the
// first directory that is not empty — another archived ticket can share the same
// nesting (_archived/github.com/acme/thing/ may hold several).
//
// Three properties make this safe, and all three are deliberate:
//
//   - it removes with os.Remove, never os.RemoveAll. os.Remove REFUSES a
//     non-empty directory, so "only prune what is empty" is enforced by the
//     syscall rather than by a check this code could get wrong. RemoveAll on a
//     path derived from a loop is how a bug here deletes somebody's archived
//     tickets.
//   - it stops at tm.archivedDir and never removes it. _archived/ is the
//     boundary, not a level: a legacy flat ticket vacates the boundary itself,
//     and an empty _archived/ is a normal state, not litter.
//   - it refuses anything that is not a real directory (os.Lstat, not os.Stat),
//     so a symlink standing in for a nesting level is left alone rather than
//     unlinked — Stat would follow it and report the target's mode.
func (tm *TaskManager) pruneEmptyArchivedDirs(dir string) {
	if tm.archivedDir == "" || dir == "" {
		return
	}
	root := filepath.Clean(tm.archivedDir)
	cur := filepath.Clean(dir)
	for cur != root {
		// Containment, re-checked every iteration: only paths strictly under
		// _archived/ are eligible, so the walk can never escape upward past the
		// boundary even if it started outside it.
		...
		if err := os.Remove(cur); err != nil {
			return // still occupied (or not ours to remove) — stop here
		}
		parent := filepath.Dir(cur)
		if parent == cur { // filesystem root; cannot happen under root, but do not spin
			return
		}
		cur = parent
	}
...
// orphanRemovalReason is the `reason` a worktree.removed event carries when the
// worktree it names belonged to no task. It is what makes the empty task_id on
// such an event readable rather than a hole a consumer has to guess about.
const orphanRemovalReason = "orphaned"

// RemoveOrphanWorktree removes a worktree that NO task owns — the sweep behind
// `adb task worktree prune` and `reconcile --prune` — and logs worktree.removed
// for it. When force is false the underlying remover refuses to discard a
// dirty/unpushed worktree (#207), exactly as it does for Cleanup.
//
// It exists because the sweepers used to call the git remover straight from the
// CLI handler, so an orphan sweep removed a worktree and logged nothing:
// metrics.WorktreesRemoved under-counted and #206's created/removed balance did
// not hold across a prune. Emission now has ONE home, next to the three
// task-owned paths (Cleanup, Archive, Delete).
//
// THE ATTRIBUTION DECISION, since an orphan has no task to hang the event on:
// the event stays `worktree.removed` — a removal is the same kind of fact
// however it was found — and carries an EMPTY task_id plus reason="orphaned".
// Additive, so no consumer breaks: every reader of data.task_id in the tree
// guards its type assertion, `adb task timeline` matches on equality with a
// non-empty id (so an orphan can never surface in a task's history), and the
// metrics fold does not read task_id for worktree events at all. A distinct
// event type was rejected: it would grow KnownEventTypes for a difference that
// is provenance, not kind, and every existing consumer of worktree.removed
// would have to learn a second name to stay correct.
func (tm *TaskManager) RemoveOrphanWorktree(worktreePath string, force bool) error {
	if worktreePath == "" {
		return fmt.Errorf("worktree path cannot be empty")
	}
	if tm.worktreeRemover == nil {
		return fmt.Errorf("no worktree remover configured")
	}
	if err := tm.worktreeRemover.RemoveWorktree(worktreePath, force); err != nil {
		// Unwrapped: the remover's own message already names the path and the
		// force override, and the CLI prints it verbatim next to the path.
		return err
	}
	if tm.eventLogger != nil {
		tm.eventLogger.Log("worktree.removed", map[string]interface{}{
...
// session (#210). Idempotent: a task that is not in backlog is left unchanged
// rather than erroring, so a caller looping over ids does not have to
...
// Close marks a task done — the counterpart to Start, and the one path both
// `adb task close` and the MCP server's adb_task_close go through.
//
// It exists as a named method rather than each caller writing
// UpdateStatus(id, done) because "closing a task" is a concept the product has,
// and two hand-rolled spellings of it are how the CLI and an agent come to mean
// slightly different things by the same word. Deliberately NOT idempotent-silent
// the way Start is: an unknown id must error, or `task close` on a typo reports
// a ticket closed that was never touched.
func (tm *TaskManager) Close(taskID string) error {
	return tm.UpdateStatus(taskID, models.TaskStatusDone)
}
