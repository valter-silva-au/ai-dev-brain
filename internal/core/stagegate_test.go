...
// TestEvaluateGate_UnknownKindIsGatedAsDeterministic pins the FAIL-SAFE for an
// item whose Kind is unset or unrecognised — the gateKind zero value, which is
// what a hand-written or future bundle gets wrong first. Such an item is gated
// on its artifact like a deterministic one, so a missing artifact still BLOCKS.
//
// This is a guard, not a new feature: `default: // deterministic` already did
// this, and the guard exists because making the switch exhaustive (adding the
// explicit `case gateDeterministic`) is exactly the edit that could quietly
// drop the default and turn an unset Kind into a silent pass — a gate item that
// waves through missing evidence is the worst possible failure mode here.
func TestEvaluateGate_UnknownKindIsGatedAsDeterministic(t *testing.T) {
	dir := t.TempDir()
	bundle := gateBundle{
		From: models.StageIdea,
		To:   models.StageMVP,
		Items: []gateItem{
			// Kind deliberately omitted (the gateKind zero value).
			{ID: "no-kind", Desc: "an item whose kind was never set", Artifact: "absent.md"},
		},
	}

	gs := evaluateGate(bundle, gateEval{evidenceDir: dir})
	if gs.Passed {
		t.Error("an item with an unset Kind and a missing artifact must BLOCK, not pass")
	}
	if len(gs.Items) != 1 {
		t.Fatalf("got %d item states, want 1", len(gs.Items))
	}
	if gs.Items[0].Status != models.GateItemMissing {
		t.Errorf("status = %q, want %q", gs.Items[0].Status, models.GateItemMissing)
	}

	// …and it is satisfiable the deterministic way, by the artifact existing.
	writeEvidence(t, dir, "absent.md", "now present")
	if gs := evaluateGate(bundle, gateEval{evidenceDir: dir}); !gs.Passed {
		t.Errorf("an unset-Kind item should be met once its artifact exists; items=%+v", gs.Items)
	}
}
