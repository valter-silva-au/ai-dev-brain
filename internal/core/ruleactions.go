...
func (r *FileActionRunner) RunExec(ctx context.Context, args []string) (string, error) {
	...
	// gosec G204 flags the variable command, correctly and unavoidably: running a
	// command the user configured IS this function. args[0] comes from the
	// workspace's own automation/rules.yaml (`adb schedule add --run-exec …`),
	// which is as trusted as any other file in the workspace — a writer who could
	// change it could equally add a git hook or edit the Taskfile.
	//
	// Two constraints keep it at that level rather than worse. NO SHELL is
	// involved: argv is passed straight to execve, so quoting/metacharacters in an
	// argument cannot become a second command. And the {{…}} expansion applied by
	// ruleEngine.runAction substitutes event payload values INTO an existing argv
	// element — it can never add an element or change which program runs.
	//nolint:gosec // G204: command comes from the workspace's own rules.yaml and is executed without a shell
