...
func realTemplateTaskManager(t *testing.T) (*TaskManager, string) {
	t.Helper()
	tempDir := t.TempDir()
	tmpl, err := NewEmbedTemplateManager(templates.FS)
	if err != nil {
	...
	tm := NewTaskManager(
		...
		NewMockTaskIDGenerator("TASK"), tmpl,
...
func readWorktreeContext(t *testing.T, task *models.Task) string {
	t.Helper()
	// The CANONICAL context, not the .claude/rules/ pointer (TASK-00039 Q5).
	p := filepath.Join(task.WorktreePath, WorktreeContextDir, WorktreeContextFile)
