...
// TestGraph_Rebuild_PersistsUnknownEdgeType is the READ half of the closed D6
// vocabulary, stated at the derived-index boundary. TASK-00039 wired
// EdgeType.Validate into every WRITE path, and the invariant it must not break is
// this one: a workspace whose frontmatter (and therefore graph/index.yaml)
// already carries a non-canonical type keeps rebuilding, keeps persisting it
// verbatim, and keeps returning it from a traversal — never an error, never a
// dropped edge. Validation belongs on write only.
func TestGraph_Rebuild_PersistsUnknownEdgeType(t *testing.T) {
	weird := models.EdgeType("mentions")
	if weird.IsCanonical() {
		t.Fatalf("%q is canonical — pick a type outside the vocabulary for this test", weird)
	}
	src := &fakeGraphSource{nodes: []GraphNode{
		{ID: "A", Links: []models.Link{
			{Type: weird, Target: "B"},
			{Type: models.EdgeRelatesTo, Target: "C"},
		}},
	}}
	store := &fakeGraphIndexStore{}
	m := NewGraphManager(src, store)
	if _, err := m.Rebuild(); err != nil {
		t.Fatalf("rebuild over a non-canonical edge type must not error: %v", err)
	}
	if store.saved == nil || len(store.saved.Edges) != 2 {
		t.Fatalf("persisted index = %+v, want both edges", store.saved)
	}
	var found bool
	for _, e := range store.saved.Edges {
		if e.Type == weird && e.From == "A" && e.To == "B" {
			found = true
		}
	}
	if !found {
		t.Errorf("persisted index dropped the non-canonical edge: %+v", store.saved.Edges)
	}
	// And it is still reachable from BOTH ends of the edge.
	for _, id := range []string{"A", "B"} {
		got, err := m.NeighborsByType(id, weird)
		if err != nil {
			t.Fatalf("NeighborsByType(%q): %v", id, err)
		}
		if len(got) != 1 {
			t.Errorf("NeighborsByType(%q, %q) = %+v, want the tolerated edge", id, weird, got)
		}
	}
}
