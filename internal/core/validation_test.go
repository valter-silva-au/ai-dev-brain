...
func TestValidationTemplates_RealEmbed(t *testing.T) {
	ts, err := ValidationTemplates(templates.FS)
	if err != nil {
		t.Fatalf("ValidationTemplates(templates.FS): %v", err)
	}
	want := []string{
...
func TestValidationPack_SatisfiesIdeaMVPGate(t *testing.T) {
	...
	dir := t.TempDir()
	if _, err := ScaffoldValidationTemplates(templates.FS, dir, HarnessInstallOptions{}); err != nil {
