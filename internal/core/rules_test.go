...
// TestRuleEngine_Outputs_RejectsNonCanonicalEdgeType is the WRITE half of the
// closed-vocabulary invariant (decision D6). `adb schedule add --write-edge
// <type>:<target>` lets a human name an edge type, and the rules file it writes
// is loaded tolerantly — so the last line of defence before a non-canonical type
// lands in an entity's frontmatter (the graph's SOURCE OF TRUTH) is the plan
// phase of fire. The rule must fail as an error firing, and — the part worth
// asserting — NOTHING may be written: the plan/apply split exists so a rejected
// output leaves no partial commit behind.
func TestRuleEngine_Outputs_RejectsNonCanonicalEdgeType(t *testing.T) {
	set := models.RuleSet{Rules: []models.Rule{
		{
			Name: "bogus-edge",
			On:   models.RuleTrigger{Event: "task.created"},
			Run:  models.RuleAction{Skill: "s"},
			Write: []models.RuleOutput{
				{Artifact: "reports/{{.task_id}}.md"},
				{Edge: &models.Link{Type: models.EdgeType("mentions"), Target: "INIT-1"}, EdgeFrom: "{{.task_id}}"},
			},
		},
	}}
	_, edges, artifacts, eng := newTestEngine(set, nil)
	f, err := eng.Dispatch(context.Background(), "task.created", map[string]string{"task_id": "TASK-7"})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
	if len(f) != 1 || f[0].Status != FiringError {
		t.Fatalf("firing = %+v, want a single error firing", f)
	}
	if !strings.Contains(f[0].Reason, "mentions") {
		t.Errorf("reason = %q, want it to name the rejected type", f[0].Reason)
	}
	if len(edges.edges) != 0 {
		t.Errorf("edges = %+v, want none written", edges.edges)
	}
	if len(artifacts.artifacts) != 0 {
		t.Errorf("artifacts = %+v, want none written (rejected in the plan phase, before any apply)", artifacts.artifacts)
	}
}

// TestRuleEngine_NonCanonicalEdgeType_ToleratedOnRead is the READ half. A rules
// file that already declares a non-canonical edge type — hand-written, or written
// by an older build — must keep LOADING and listing. Only the write is refused
// (covered above); a workspace never loses visibility of a rule it owns.
func TestRuleEngine_NonCanonicalEdgeType_ToleratedOnRead(t *testing.T) {
	set := models.RuleSet{Rules: []models.Rule{
		{
			Name:  "legacy",
			On:    models.RuleTrigger{Event: "task.created"},
			Run:   models.RuleAction{Skill: "s"},
			Write: []models.RuleOutput{{Edge: &models.Link{Type: models.EdgeType("mentions"), Target: "INIT-1"}, EdgeFrom: "TASK-1"}},
		},
	}}
	_, _, _, eng := newTestEngine(set, nil)
	rules, err := eng.Rules()
	...
	if len(rules) != 1 || rules[0].Name != "legacy" {
		t.Fatalf("Rules() = %+v, want the legacy rule preserved", rules)
	}
	if got := rules[0].Write[0].Edge.Type; got != models.EdgeType("mentions") {
		t.Errorf("edge type = %q, want it preserved verbatim on read", got)
	}
}
