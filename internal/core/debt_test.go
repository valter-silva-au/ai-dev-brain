...
func TestDebtManager_AddListResolve(t *testing.T) {
	...
	if _, err := m.Add("slow query", "storage", "", models.PriorityP1, nil); err != nil {
		t.Fatalf("Add: %v", err)
	}
	if _, err := m.Add("no default", "", "priority defaults to P2", "", nil); err != nil {
		t.Fatalf("Add: %v", err)
	}
	crit, err := m.Add("panics on nil", "core", "", models.PriorityP0, nil)
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
...
func TestDebtManager_Add_RejectsInvalidPriority(t *testing.T) {
	...
	for _, bad := range []models.Priority{"p0", "P4", "NONSENSE", "high"} {
		if _, err := m.Add("critical thing", "core", "", bad, nil); err == nil {
	...
	for _, ok := range []models.Priority{models.PriorityP0, models.PriorityP1, models.PriorityP2, models.PriorityP3, ""} {
		if _, err := m.Add("fine", "core", "", ok, nil); err != nil {
			t.Errorf("Add with valid priority %q should succeed, got %v", ok, err)
		}
	}
}

// TestDebtManager_Add_StoresLinks covers the graph half of a debt item: like an
// ADR, it carries the typed links it declares, and those links are what the
// GraphSource turns into edges incident to its debt:DEBT-NNNN node.
func TestDebtManager_Add_StoresLinks(t *testing.T) {
	store := &memDebtStore{}
	m := NewDebtManager(store, WithDebtClock(fixedClock()))

	item, err := m.Add("panics on nil graph", "core", "", models.PriorityP0,
		[]models.Link{{Type: models.EdgeRelatesTo, Target: "TASK-1"}})
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
	if len(item.Links) != 1 || item.Links[0].Type != models.EdgeRelatesTo || item.Links[0].Target != "TASK-1" {
		t.Errorf("returned links = %+v", item.Links)
	}
	// Persisted, not just returned — the registry is the graph's source of truth.
	stored, err := m.List()
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(stored) != 1 || len(stored[0].Links) != 1 || stored[0].Links[0].Target != "TASK-1" {
		t.Errorf("stored links = %+v", stored)
	}
	if got := stored[0].GraphID(); got != "debt:DEBT-0001" {
		t.Errorf("graph id = %q, want debt:DEBT-0001", got)
	}

	// No links is the ordinary case and stays nil (a degree-0 graph node).
	plain, err := m.Add("no links", "", "", models.PriorityP2, nil)
	if err != nil {
		t.Fatalf("Add: %v", err)
	}
	if plain.Links != nil {
		t.Errorf("links = %+v, want nil", plain.Links)
	}
}
