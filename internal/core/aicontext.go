...
// AIContextGenerator generates the workspace's agent instruction files.
type AIContextGenerator interface {
	Generate() error
	// GenerateInto publishes the same content and reports what happened to each
	// file, so a caller can surface skipped hand-written files.
	GenerateInto(pointers []string, force bool) ([]InstructionFileResult, error)
}
...
// Generate publishes the instruction files using the default pointer set.
func (g *DefaultAIContextGenerator) Generate() error {
	_, err := g.GenerateInto(DefaultInstructionPointers(), false)
	return err
}

// GenerateInto assembles the context and publishes it as the canonical
// AGENTS.md plus one pointer file per configured harness.
func (g *DefaultAIContextGenerator) GenerateInto(
	pointers []string,
	force bool,
	...
	// Publish it as the canonical AGENTS.md plus per-harness pointers.
	results, err := WriteInstructionFiles(g.repoRoot, content, pointers, force)
	if err != nil {
		return results, fmt.Errorf("failed to write instruction files: %w", err)
	}
	...
	if err := g.saveContextState(newState); err != nil {
		return results, fmt.Errorf("failed to save context state: %w", err)
	}
...
├── .adb/                  # Per-workspace state (events, counters, context state)
├── AGENTS.md             # This file — the canonical agent instructions
└── CLAUDE.md             # Pointer to AGENTS.md, for agents that look for it
` + "```" + `
`
}
...
func (g *DefaultAIContextGenerator) generateRecentSessions() string {
	...
	// Find all session files. The modification time is captured HERE, from the
	// FileInfo the walk already carries, rather than re-Stat'ed inside the sort
	// comparison: a path the walk yielded is not guaranteed to be Stat-able
	// later (a dangling symlink under a sessions/ dir, or a file deleted between
	// the walk and the sort), and a nil FileInfo made `ModTime()` a nil-pointer
	// panic that took the whole context build down over one unreadable file.
	ticketsDir := filepath.Join(g.repoRoot, "tickets")
	// Field order is modTime-first for fieldalignment, not for meaning.
	...
	err := filepath.Walk(ticketsDir, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			// Continue on error: one unreadable ticket subtree must not cost the
			// whole Recent Sessions section. info is nil here, so this must
			// return before it is dereferenced.
			//nolint:nilerr // best-effort walk — an unreadable entry is skipped, not fatal
			return nil
		}
	...
	for i := 0; i < limit; i++ {
		...
		// Fall back to the absolute path when the workspace-relative one cannot
		// be computed. Unreachable for a path the walk produced under repoRoot,
		// but an ignored error here used to mean an empty heading (`### `) in the
		// generated file — a silently anonymous session is worse than a long path.
		relPath, relErr := filepath.Rel(g.repoRoot, file)
		if relErr != nil {
			relPath = file
		}
...
func (g *DefaultAIContextGenerator) generateWhatsChanged(prevHashes, currentHashes map[string]string) string {
	...
	// An ORDERED slice, not a map. Ranging a Go map randomizes iteration order, so
	// this list came out in a different order on every run and CLAUDE.md — a
	// git-tracked file — was non-deterministic whenever more than one section had
	// changed. Two regenerations from identical inputs produced different bytes.
	// The order below matches assembleCLAUDEmd's section order, so "What's Changed"
	// reads in the same order as the document it describes.
	sectionNames := []struct {
		key  string
		name string
	}{
	...
	for _, section := range sectionNames {
		prevHash, hadPrev := prevHashes[section.key]
		currHash, hasCurr := currentHashes[section.key]

		if !hadPrev && hasCurr {
			changes = append(changes, fmt.Sprintf("- **%s**: New section added", section.name))
		} else if hadPrev && hasCurr && prevHash != currHash {
			changes = append(changes, fmt.Sprintf("- **%s**: Updated", section.name))
		}
	}
...
func (g *DefaultAIContextGenerator) assembleCLAUDEmd(sections map[string]string) string {
	...
	// Footer.
	//
	// Deliberately carries NO generation timestamp. CLAUDE.md is a git-tracked
	// artifact, so a `time.Now()` in it made every regeneration produce a diff even
	// when nothing had actually changed — spurious churn that trains you to ignore
	// the one file whose diffs you most want to read. The generation time is not
	// lost: ContextState.LastGenerated records it in .adb/context_state.yaml, which
	// is state rather than content and where churn is free. Keeping the body free of
	// non-deterministic input is what makes regeneration idempotent (see
	// TestSyncContext_IsIdempotentAfterFirstRun).
	sb.WriteString("\n---\n\n")
	sb.WriteString("_Generated by AI Dev Brain — see .adb/context_state.yaml for the generation time._\n")
...
func (g *DefaultAIContextGenerator) loadContextState() (*ContextState, error) {
	statePath := statedir.Path(g.repoRoot, statedir.FileContextState)
...
func (g *DefaultAIContextGenerator) saveContextState(state *ContextState) error {
	...
	// Ensure the .adb/ state dir exists (#186) — this generator can be
	// constructed and run without going through NewApp's ensureStateDir.
	if err := statedir.Ensure(g.repoRoot); err != nil {
		return err
	}
