...
func TestComplianceFrameworks_FromEmbed(t *testing.T) {
	frameworks, err := ComplianceFrameworks(templates.FS)
	if err != nil {
...
func TestScaffoldComplianceFramework(t *testing.T) {
	dir := t.TempDir()
	res, err := ScaffoldComplianceFramework(templates.FS, "soc2", dir, HarnessInstallOptions{})
	if err != nil {
	...
	res2, err := ScaffoldComplianceFramework(templates.FS, "soc2", dir, HarnessInstallOptions{})
	if err != nil {
	...
	if _, err := ScaffoldComplianceFramework(templates.FS, "pci", dir, HarnessInstallOptions{}); err == nil {
