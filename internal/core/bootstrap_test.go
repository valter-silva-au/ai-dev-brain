...
func TestBootstrapSystem(t *testing.T) {
	...
	tm, err := NewEmbedTemplateManager(templates.FS)
	if err != nil {
	...
	// Verify the canonical task context was created under .adb/ (TASK-00039 Q5:
	// the .claude/rules/ path is now a pointer to it, not the content).
	...
	expectedTaskContextFile := filepath.Join(tempDir, WorktreeContextDir, WorktreeContextFile)
...
func TestBootstrapSystemFileContents(t *testing.T) {
	...
	// Test notes.md content — TASK-00031: notes.md is an append-only session
	// log; it must NOT carry the description (that is context.md's job) and it
	// must carry the frontmatter stamp + session-log seed instead.
	...
	// The duplicate-seed fix: the description (and criteria) live ONLY in
	// context.md now.
	for _, forbidden := range []string{"Testing file contents", "Must have content A"} {
		if strings.Contains(notesStr, forbidden) {
			t.Errorf("notes.md still carries the description seed %q (duplicate).\nContent:\n%s", forbidden, notesStr)
		}
	}
...
func TestBootstrapSystemEmptyAcceptanceCriteria(t *testing.T) {
	...
	// TASK-00031: the session log is the notes seed; the criteria placeholder
	// moved to context.md (notes.md no longer renders acceptance criteria).
	notesContent, err := os.ReadFile(result.NotesFile)
	if err != nil {
		t.Fatalf("Failed to read notes file: %v", err)
	}
	notesStr := string(notesContent)
	if !strings.Contains(notesStr, "## Session Log") {
		t.Errorf("Expected notes.md to seed a session log.\nContent:\n%s", notesStr)
	}
...
func TestBootstrapSystemDirectoryStructure(t *testing.T) {
	...
	expectedDirs := []string{
		...
		filepath.Join(tempDir, WorktreeContextDir),
	...
	expectedFiles := []string{
		...
		filepath.Join(tempDir, WorktreeContextDir, WorktreeContextFile),
	}
...
func TestGenerateTaskContext(t *testing.T) {
	...
	// Verify file exists
	taskContextPath := filepath.Join(tempDir, WorktreeContextDir, WorktreeContextFile)
...
// the worktree task-context.md showed the literal "<no value>", fell through
...
func tier0Fixture() BootstrapConfig {
	// Inline the real standing rules so the Tier-0 (b) block asserts the actual
	// shipped content rather than the terse fallback stub (TASK-00031 split).
	rules := ""
...
func TestTaskContext_Tier0AllBlocks(t *testing.T) {
	...
	content, err := os.ReadFile(filepath.Join(tempDir, WorktreeContextDir, WorktreeContextFile))
	if err != nil {
		t.Fatalf("Failed to read task-context.md: %v", err)
	}
	rendered := string(content)
	...
	for _, want := range []string{
		...
		"Definition of Done",
		"Reply as the workspace owner", // from standing-dos-donts.md
		"Branch discipline",            // from standing-dos-donts.md
		"Production safety",            // from standing-dos-donts.md
		// From standing-dos-donts.md. Asserted as the TIER, not a version string: the
		// rule used to pin an exact model version and so fired on every newer session.
		"strongest configured tier or better",
	} {
		if !strings.Contains(rendered, want) {
