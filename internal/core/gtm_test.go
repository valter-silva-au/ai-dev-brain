...
func TestGTMPacks_FromEmbed(t *testing.T) {
	packs, err := GTMPacks(templates.FS)
	if err != nil {
...
func TestScaffoldGTMPack(t *testing.T) {
	dir := t.TempDir()
	res, err := ScaffoldGTMPack(templates.FS, "moat", dir, HarnessInstallOptions{})
	if err != nil {
	...
	// Unknown pack errors.
	if _, err := ScaffoldGTMPack(templates.FS, "nope", dir, HarnessInstallOptions{}); err == nil {
