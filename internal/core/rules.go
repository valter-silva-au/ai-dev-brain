...
func (e *ruleEngine) applyOutputs(rule models.Rule, payload map[string]string, firingEntity string) error {
	...
	for i, o := range rule.Write {
		...
		if o.Edge != nil {
			// The closed D6 vocabulary, gated in the PLAN phase. `adb schedule add`
			// rejects a non-canonical --write-edge type at authoring time, but
			// automation/rules.yaml is loaded tolerantly (a hand-written or
			// older-build rule keeps listing and firing), so this is the last point
			// before an unknown type is minted onto an entity's frontmatter — the
			// graph's source of truth. Failing here rather than in APPLY is what
			// keeps a rejected output from leaving a half-written artifact behind.
			if err := o.Edge.Type.Validate(); err != nil {
