...
// TestPush_ManifestObjectCarriesNoCredential is the end-to-end guard on the
// bytes that actually reach the bucket: it reads the uploaded
// repos-manifest.tsv object back out of the store and asserts the credential
// is not in it.
//
// It runs with a CLEAN leak runner on purpose. Before the fix, the only thing
// between an embedded token and S3 was the fail-closed gitleaks scan noticing
// it in the staging copy — real mitigation, but incidental: it depends on
// staging order, and it turns a routine push into a hard abort instead of
// doing the right thing with the URL. Stubbing the scanner clean is what makes
// this test measure the fix rather than the scanner.
func TestPush_ManifestObjectCarriesNoCredential(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not on PATH")
	}
	root := buildFixtureWorkspace(t)
	// repos/ is denied for upload, but GenerateManifest reads it — which is
	// exactly how the origin gets into an uploaded object.
	mkRepoWithOrigin(t, filepath.Join(root, "repos"), "github.com/org/repo",
		"https://svcuser:"+fakeToken+"@github.com/org/repo.git")
	...
	cfg := Config{
		...
		Region:   "ap-southeast-2",
		Store:    store,
		// Deliberately clean: assert the fix, not the scanner.
		Leak: func(_ ...string) ([]byte, int, error) { return []byte("clean"), 0, nil },
	}
	if err := Push(context.Background(), cfg); err != nil {
		t.Fatalf("Push: %v", err)
	}

	rc, err := store.Get(context.Background(), "repos-manifest.tsv")
	if err != nil {
		t.Fatalf("uploaded manifest missing: %v", err)
	}
	defer rc.Close()
	...
	if strings.Contains(body, fakeToken) {
		t.Errorf("the UPLOADED manifest object contains the credential:\n%s", body)
	}
	if strings.Contains(body, "@") {
		t.Errorf("the UPLOADED manifest object still carries userinfo:\n%s", body)
	}
	// And it is still worth uploading: the row identifies a clone-able repo.
	if !strings.Contains(body, "github.com/org/repo\thttps://github.com/org/repo.git\t") {
		t.Errorf("uploaded manifest lost the clone-able origin:\n%s", body)
	}
}
