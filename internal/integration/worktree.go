...
type GitWorktreeManager interface {
	...
	// is recreated from baseBranch. Backs `adb task worktree reconcile` (#211).
...
func resolveWorktreeBase(repoDir, baseBranch string) string {
	...
	fetch.Dir = repoDir
	//nolint:errcheck // by design (documented above): the fetch is a freshness
	// optimisation, and every failure mode (offline, suppressed auth prompt,
	// deleted upstream branch) is handled by the refExists fallback below, which
	// branches from whatever origin/<base> is already local. Surfacing the error
	// here would break worktree creation for an offline developer.
	_ = fetch.Run()
...
func worktreeDirtyReason(worktreePath string) (string, error) {
	...
	// repo has nowhere to push, so its commits are not "unpushed". A FAILED
	// probe is a different thing entirely and must not share that verdict:
	// this used to be `if err != nil || out == ""`, so an unanswerable `git
	// remote` reported the worktree clean and #207 waved through a teardown it
	// could not verify. RemoveWorktree turns an error into a refusal naming
	// force, which is the fail-safe direction and matches this function's
	// contract (empty reason + nil error means "safe to remove").
...
// the rebuild primitive behind `adb task worktree reconcile` (#211).
...
func parseStatusBranchLine(s string) (branch string, ahead, behind int) {
	if i := strings.Index(s, " ["); i >= 0 {
		...
		for _, part := range strings.Split(bracket, ", ") {
			// An unparseable count leaves the counter at 0 — the same outcome the
			// discarded fmt.Sscanf error produced, now stated rather than implied.
