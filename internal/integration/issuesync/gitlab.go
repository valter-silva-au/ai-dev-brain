...
func (p *gitlabProvider) Update(owner, name string, number int, want RemoteIssue) (RemoteIssue, error) {
	...
	// State reconcile: not optional. The open/closed transition is how the
	// `status` synced field lands on the remote (see StatusToState), so a
	// discarded error here made a failed close report a successful sync —
	// Task.SyncHash was refreshed to the local state and nothing retried.
	// Update is idempotent, so the next `adb issues sync` converges.
	verb := "reopen"
	if want.State == IssueClosed {
		verb = "close"
	}
	if _, err := p.run("issue", verb, strconv.Itoa(number), "-R", owner+"/"+name); err != nil {
