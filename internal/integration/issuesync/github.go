...
// used by internal/integration/reposync_walk.go for `git`.
...
func (p *githubProvider) Update(owner, name string, number int, want RemoteIssue) (RemoteIssue, error) {
	...
	// State reconcile: close or reopen to match the desired state. This is not
	// optional — the open/closed transition is how the `status` synced field
	// lands on the remote (see StatusToState), so discarding its error made a
	// failed close look like a successful sync: the syncer refreshed
	// Task.SyncHash to the local state and no later run retried, leaving adb
	// believing the issue was closed while it was still open. Update is
	// idempotent, so reporting the failure lets the next `adb issues sync`
	// converge and surfaces the reason in the Result an operator reads.
	verb := "reopen"
	if want.State == IssueClosed {
		verb = "close"
	}
	if _, err := p.run("issue", verb, strconv.Itoa(number), "--repo", owner+"/"+name); err != nil {
		return RemoteIssue{}, fmt.Errorf("reconcile issue state (%s): %w", verb, err)
	}
	return want, nil
}
