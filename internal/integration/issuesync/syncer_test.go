...
// TestSyncer_SkipsInternalTenant — a non-github/gitlab internal ticket host goes
// through the same skip path (not github, not gitlab).
func TestSyncer_SkipsInternalTenant(t *testing.T) {
	var logged []loggedEvent
	s := &Syncer{
		Body:  func(models.Task) string { return "" },
		Write: func(models.Task) error { return nil },
	...
	res := s.SyncTask(models.Task{ID: "TASK-2", Repo: "code.internal-corp.dev/team/thing"}, DirectionBoth, false)
	if res.Action != ActionNoop {
		t.Fatalf("internal-tenant ticket should be noop, got %q", res.Action)
	}
	if len(logged) != 1 || logged[0].Event != string(observability.EventIssueSkipped) {
