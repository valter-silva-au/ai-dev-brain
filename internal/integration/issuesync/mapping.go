...
func StatusToState(s models.TaskStatus) IssueState {
	...
	case models.TaskStatusDone, models.TaskStatusArchived:
		return IssueClosed
	case models.TaskStatusBacklog, models.TaskStatusInProgress,
		models.TaskStatusBlocked, models.TaskStatusReview:
		return IssueOpen
	default:
		// An unrecognised status (a hand-edited backlog.yaml, or a status added
		// to models without being classified here) keeps the issue open. Closing
		// is the destructive direction, so the unknown case must not take it.
		// The four live statuses are enumerated above rather than folded into
		// this default so adding a TaskStatus fails the exhaustive linter and
		// forces the open/closed decision to be made deliberately.
		return IssueOpen
	}
}
