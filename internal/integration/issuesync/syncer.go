...
func (s *Syncer) SyncTask(tk models.Task, dir Direction, dryRun bool) Result {
	...
	case ActionNoop:
		// Unreachable: the dryRun/ActionNoop guard above already returned.
		// Enumerated so adding an Action fails the exhaustive linter here rather
		// than silently falling through and skipping its write.
