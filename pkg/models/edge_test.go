...
// TestEdgeType_Validate covers the WRITE-side primitive: Validate wraps
// IsCanonical and is what a write surface calls, so its error must name the
// offending type AND the closed vocabulary the author is allowed to pick from —
// a bare "invalid edge type" leaves them guessing at a set they cannot see.
func TestEdgeType_Validate(t *testing.T) {
	for _, et := range CanonicalEdgeTypes {
		if err := et.Validate(); err != nil {
			t.Errorf("EdgeType(%q).Validate() = %v, want nil", et, err)
		}
	}
	for _, et := range []EdgeType{"", "relatesto", "child_of", "RELATES_TO", "depends-on", "mentions"} {
		err := et.Validate()
		if err == nil {
			t.Fatalf("EdgeType(%q).Validate() = nil, want a rejection", et)
		}
		msg := err.Error()
		if !strings.Contains(msg, string(EdgeRelatesTo)) || !strings.Contains(msg, string(EdgeDuplicates)) {
			t.Errorf("EdgeType(%q).Validate() = %q; want it to name the canonical set", et, msg)
		}
		if et != "" && !strings.Contains(msg, string(et)) {
			t.Errorf("EdgeType(%q).Validate() = %q; want it to name the offending type", et, msg)
		}
	}
}
