...
type TemplateAnswers struct {
	...
	// WithPrograms lists the document-program pack ids provisioned into the
	// project (TASK-00023). It is recorded — rather than re-derived from whatever
	// happens to be on disk — so `adb init update` re-renders exactly the packs the
	// project opted into and can three-way diff their files like any other
	// scaffolded artifact. An empty list means no packs were provisioned.
	WithPrograms []string `yaml:"with_programs,omitempty" json:"with_programs,omitempty"`

	// NoPrograms records an EXPLICIT opt-out (`adb init project --no-programs`).
	//
	// It exists because an empty WithPrograms is ambiguous: it describes both "this
	// project deliberately wanted no packs" and "this project predates document
	// programs entirely". Those two need opposite treatment on `adb init update` —
	// the first must be left alone, the second is retro-provisioned with the default
	// pack (decision D7). Recording the opt-out is what separates them, so the
	// update path never has to guess from the absence of data.
	NoPrograms bool `yaml:"no_programs,omitempty" json:"no_programs,omitempty"`
}
