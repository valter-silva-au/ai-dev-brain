...
const (
	...
	// still carry it, and ConventionalType still maps it to "fix" so such a row
	// keeps producing a correct branch name. It is deliberately NOT in
	// ValidTaskTypes — the create path and MCP server reject it with a
	// "use `fix`" hint so no new bug-typed tasks are minted.
	//
	// Retyping an existing row is a manual edit to backlog.yaml: the
	// `adb task migrate-types` command that used to do it was removed with the
	// other one-shot migrations.
...
// ValidTaskStatuses is the closed set of the six lifecycle states, in lifecycle
// order. Callers that accept a status from a human (a --filter/--status flag)
// should validate against it rather than comparing strings and silently matching
// nothing — "no tasks found" is a wrong answer for a typo.
...
// IsValidTaskStatus reports whether s is one of the six lifecycle states.
...
// ValidTaskStatusNames returns the six status names, for an error message that
// tells the caller what it should have said.
func ValidTaskStatusNames() []string {
	names := make([]string, 0, len(ValidTaskStatuses))
	for _, v := range ValidTaskStatuses {
...
// docs, chore, test, perf) maps 1:1 to its own prefix; the non-identity mappings
// are: legacy bug -> fix and spike -> chore (fixed by the
// adb-correlation-layout ADR), and prototype -> chore (a throwaway experiment
// is chore-shaped, like spike). The non-code `work` type maps to itself like the
// code types, but the value is never used: work never mints a branch
// (TaskManager.Create skips worktree/branch creation for it).
//
// Every TaskType is named in the switch so that adding a type to the taxonomy
// cannot silently inherit an identity mapping — the exhaustive linter fails the
// build until the new type is classified here. The default remains for an
// unknown/empty type string and returns it verbatim, so a hand-edited
// backlog.yaml never breaks branch creation.
