...
func (p EntityProposal) Validate() error {
	...
	case ProposalEdge:
		...
		// Validate() runs on the WRITE path only (ingestManager.Submit), never on
		// read, so gating the closed D6 vocabulary here keeps an extraction skill's
		// invented type — "mentions", "owns" — out of entity frontmatter while a
		// queue.yaml that already carries one still lists.
		if err := p.Edge.Type.Validate(); err != nil {
			return fmt.Errorf("proposal %q: %w", p.ID, err)
		}
	case ProposalNode:
