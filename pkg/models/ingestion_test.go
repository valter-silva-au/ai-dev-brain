...
func TestEntityProposal_Validate(t *testing.T) {
	...
	bad := []struct {
		...
		// A proposal is authored by an extraction skill — the surface most likely
		// to invent a plausible-sounding edge type. Validate() runs on the WRITE
		// path only (ingestManager.Submit), so gating the closed #109 vocabulary
		// here keeps a non-canonical type out of entity frontmatter without
		// touching read tolerance.
		{"edge type not canonical", EntityProposal{ID: "x", RawID: "r", Kind: ProposalEdge, Confidence: 0.5, From: "T", Edge: &Link{Type: EdgeType("mentions"), Target: "U"}}},
		{"edge type near-miss typo", EntityProposal{ID: "x", RawID: "r", Kind: ProposalEdge, Confidence: 0.5, From: "T", Edge: &Link{Type: EdgeType("depends-on"), Target: "U"}}},
	}
	for _, tc := range bad {
		t.Run(tc.name, func(t *testing.T) {
