...
// outputs.
//
// It is deliberately STRUCTURAL: it does not check that On.Event is a known event
// type, nor that If.HasEdge / a Write edge is a canonical EdgeType. Both
// vocabularies are gated at the WRITE surface instead — `adb schedule add`, in
// internal/cli/schedule.go's buildRuleFromFlags (validateEventType for the event
// type, EdgeType.Validate for the edge types) — while READ stays tolerant, the way
// the graph tolerates unknown edge types (see the EdgeType doc comment).
//
// That split is load-bearing, not stylistic. RuleSet.Validate calls this on every
// rule, and FileRuleStore.Save validates the whole set, so a known-type check here
// would make one legacy or hand-edited rule reject the entire automation/rules.yaml
// — `adb schedule list` would error and `adb schedule remove` could not even delete
// the offending rule. Keep vocabulary checks out of this function.
func (r Rule) Validate() error {
	...
	case r.On.IsEvent():
		// Structural OK. The known-type check is `adb schedule add`'s
		// (cli.validateEventType) — see this method's doc comment for why it must
		// not move here.
	default:
