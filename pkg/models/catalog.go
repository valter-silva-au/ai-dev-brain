...
// read-only snapshot — orgs, initiatives, tickets, ingested nodes, metric nodes,
// ADRs, and tech-debt items — each annotated with its graph degree (the count of
// incident edges) so the catalog doubles as a graph overview. It carries no
// timestamp so the same workspace always yields byte-identical output
// (deterministic for diffs/tests); callers that want a "generated at" line stamp
// it at render time.
type Catalog struct {
	...
	Debt          []CatalogDebt       `yaml:"debt" json:"debt"`
...
// CatalogDebt is one tech-debt item's catalog entry, keyed by its graph id. It
// carries the two fields triage is done on (priority, status) so the catalog can
// answer "what debt is outstanding, and how connected is it?" without a second
// read of debt/index.yaml.
type CatalogDebt struct {
	ID       string `yaml:"id" json:"id"` // the debt graph id (debt:DEBT-NNNN)
	Title    string `yaml:"title" json:"title"`
